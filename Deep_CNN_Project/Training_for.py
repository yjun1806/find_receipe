...
for x in range(iterate_train):
    ...
    for a in range(10):
        avg_inference_time += train_util.display_prediction(random_test_image(), model, topk=3, model_name=model_choice)
        time.sleep(1.1)
    ...
    results.sort_values('top3', ascending=False, inplace=True)
